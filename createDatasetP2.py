...
if __name__ == '__main__': 
    ...
    with open(file_list, 'r') as names:
        for name in names:
            occ_level, bg_img_path = name.strip().split(' ')
            img_id = bg_img_path.split('/')[-1].split('.')[0]

            bg_mask_path = os.path.join(bg_dir, img_id + '_mask.png')

            ref_img_path = os.path.join(fg_dir, img_id + '.jpg')
            ref_mask_path = os.path.join(fg_dir, img_id + '_mask.png')
            ref_img = cv2.imread(ref_img_path)

            ref_mask = (cv2.imread(ref_mask_path) > 128).astype(np.uint8)[:,:,0]

            bg_mask = Image.open(bg_mask_path).convert('L')
            bg_mask = np.array(bg_mask)
            bg_mask = bg_mask == 255

            bg_img = cv2.imread(bg_img_path)
            gen_image = inference_single_image(ref_img, ref_mask, bg_img, bg_mask)
            gen_path = os.path.join(save_dir, 'images', occ_level, img_id + '.jpg')
